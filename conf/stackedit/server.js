...
if(!process.env.NO_CLUSTER && cluster.isMaster) {
        ...
        var host = process.env.VCAP_APP_HOST || process.env.HOST || "0.0.0.0" || "localhost";
        var port = process.env.VCAP_APP_PORT || process.env.PORT || 80;
        if(port == 443) {
                ...
                httpsServer.listen(port, null, function() {
                        console.log('HTTPS server started: https://' + host);
                });
                port = process.env.VCAP_APP_PORT || process.env.PORT || 80;
        }
